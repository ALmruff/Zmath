...
pygame.display.set_caption("ZMATH: GALACTIC TARGET - CARRIER EDITION")
...
class Shockwave:
    def __init__(self, x, y, color, speed=5):
        ...
        self.radius = 5; self.speed = speed; self.alpha = 255
...
# --- 5. LOGIKA ATURAN 25 LEVEL ---
class Alien:
    def __init__(self, target_pos, level, prefix):
        self.target_pos = target_pos
        self.prefix = prefix 
        self.generate_mission(level)

    def generate_mission(self, level):
        op = '+'
        self.spawn_delay_modifier = 0 
        speed_grade = 1.0 

        if level == 1:
            op = '+'
            ans = random.randint(2, 20)
            self.a = random.randint(1, ans - 1)
            self.b = ans - self.a
            speed_grade = 1.0
        elif level == 2:
            op = '-'
            ...
            self.a = random.randint(5, ans - 5)
            self.b = ans - self.a
            speed_grade = 1.0
            self.spawn_delay_modifier = 800 
        elif level == 7:
            op = '+'
            ans = random.randint(10, 50)
            self.a = random.randint(5, ans - 5)
            ...
            ans = self.a - self.b
            speed_grade = 0.8
            self.spawn_delay_modifier = 1200
        elif level == 11:
            op = '+'
            ans = random.randint(40, 100)
            self.a = random.randint(10, ans - 10)
            self.b = ans - self.a
            speed_grade = 0.9
            self.spawn_delay_modifier = 1500
        elif level == 12:
            op = '-'
            ...
            if op == '+':
                ...
                ans = self.a - self.b
            speed_grade = 1.2
            self.spawn_delay_modifier = 1000
        elif level == 14:
            op = random.choice(['+', '-'])
            if op == '+':
                ans = random.randint(20, 80)
                self.a = random.randint(5, ans - 5)
                self.b = ans - self.a
            else:
                self.a = random.randint(5, 15)
                self.b = random.randint(16, 30)
                ans = self.a - self.b
            speed_grade = 1.1
            self.spawn_delay_modifier = 1200
        elif level == 15:
            ...
            ans = self.a * self.b
            speed_grade = 0.8
            self.spawn_delay_modifier = 1800
        elif level == 16:
            op = 'x'
            self.a = random.randint(2, 9)
            self.b = random.randint(2, 9)
            ans = self.a * self.b
            speed_grade = 1.1
            self.spawn_delay_modifier = 1400
        elif level == 17:
            op = ':'
            ans = random.randint(2, 20) 
            self.b = random.randint(2, 9)  
            self.a = ans * self.b          
            speed_grade = 0.8
            self.spawn_delay_modifier = 2000
        elif level == 18:
            op = ':'
            ans = random.randint(2, 20)
            self.b = random.randint(2, 9)
            ...
            op = random.choice(['+', '-', 'x', ':'])
            if op == '+':
                ans = random.randint(10, 80)
                self.a = random.randint(1, ans - 1)
                self.b = ans - self.a
            elif op == '-':
                self.a = random.randint(20, 70)
                self.b = random.randint(1, 19)
                ans = self.a - self.b
            elif op == 'x':
                self.a = random.randint(2, 9)
                self.b = random.randint(2, 9)
            ...
            if op == '+':
                ...
                ans = random.randint(5, 25)
                self.b = random.randint(3, 11)
                self.a = ans * self.b
            speed_grade = 1.4
            self.spawn_delay_modifier = 800

        self.answer = str(ans)
        self.question = f"{self.a}{op}{self.b}"
        ...
        self.vx, self.vy = math.cos(self.angle) * actual_speed, math.sin(self.angle) * actual_speed
        self.input = ""
    ...
    def draw(self, surf, is_active):
        ...
        thrust_x = self.x + math.cos(thrust_angle) * 18
        thrust_y = self.y + math.sin(thrust_angle) * 18
        ...
        pygame.draw.ellipse(surf, WHITE, (self.x - 10, self.y - 16, 12, 5)) 
        ...
        surf.blit(frame_surf, frame_rect.topleft)
        surf.blit(txt, rect)

# --- 6. FUNGSI GARIS PENARGETAN PUTUS-PUTUS (DASHED LINE) ---
def draw_dashed_target_line(surf, color, start_pos, end_pos, dash_length=12, space_length=8, width=2):
    ...
    total_segment = dash_length + space_length
    num_dashes = int(distance / total_segment)
    
    for i in range(num_dashes + 1):
        start_d = i * total_segment
        end_d = start_d + dash_length
...
# --- 7. RENDER KAPAL INDUK & TURRET YANG BEROTASI ---
def draw_massive_carrier(surf, center_pos, target_alien):
    if target_alien:
        ...
        barrel_tip_x = center_pos[0] + math.cos(angle_laser) * 50
        barrel_tip_y = center_pos[1] + math.sin(angle_laser) * 50
        
        target_radius = 45 
        end_target_x = target_alien.x - math.cos(angle_laser) * target_radius
        end_target_y = target_alien.y - math.sin(angle_laser) * target_radius
    ...
    p_ticks = pygame.time.get_ticks()
    light_color = CYAN if p_ticks % 1000 < 500 else GRAY
    pygame.draw.rect(surf, light_color, (WIDTH // 2 - 180, HEIGHT - 25, 10, 5), border_radius=2)
    pygame.draw.rect(surf, light_color, (WIDTH // 2 + 170, HEIGHT - 25, 10, 5), border_radius=2)

    if target_alien:
        angle = math.atan2(target_alien.y - center_pos[1], target_alien.x - center_pos[0])
    else:
        angle = -math.pi / 2
    ...
    barrel_points = [
        rotate_point((50, -6), angle),
        rotate_point((50, 6), angle),
        rotate_point((-5, 6), angle),
        rotate_point((-5, -6), angle)
    ]
    
    base_t_points = [
        rotate_point((30, -22), angle),
        rotate_point((30, 22), angle),
        rotate_point((-30, 22), angle),
        rotate_point((-30, -22), angle)
    ]
    ...
    mount_points = [
        (center_pos[0] - 40, center_pos[1] + 15),
        (center_pos[0] + 40, center_pos[1] + 15),
        (center_pos[0] + 55, center_pos[1] + 45),
        (center_pos[0] - 55, center_pos[1] + 45)
    ]
    pygame.draw.polygon(surf, (20, 20, 30), mount_points)
    pygame.draw.polygon(surf, WHITE, mount_points, 1)

    pygame.draw.polygon(surf, (30, 30, 50), base_t_points)
    pygame.draw.polygon(surf, WHITE, base_t_points, 2)
    
    pygame.draw.polygon(surf, CYAN, barrel_points)
    pygame.draw.polygon(surf, WHITE, barrel_points, 1)
    
    cockpit_pts = [
        rotate_point((18, 0), angle),
        rotate_point((4, -3), angle),
        rotate_point((-2, 0), angle),
        rotate_point((4, 3), angle)
    ]
    pygame.draw.polygon(surf, CYAN, cockpit_pts)
    pygame.draw.polygon(surf, WHITE, cockpit_pts, 1)
...
def main():
    ...
    while menu_running:
        ...
        txt_sub = font_math.render("ENTER YOUR PILOT NAME:", True, WHITE)
        txt_user = font_big.render(username + ("_" if pygame.time.get_ticks() % 1000 < 500 else ""), True, YELLOW)
        txt_start = font_ui.render("Press ENTER to Launch Mission", True, GRAY)

        screen.blit(txt_title, txt_title.get_rect(center=(WIDTH//2, HEIGHT//2 - 150)))
        screen.blit(txt_sub, txt_sub.get_rect(center=(WIDTH//2, HEIGHT//2 - 40)))
        screen.blit(txt_user, txt_user.get_rect(center=(WIDTH//2, HEIGHT//2 + 50)))
        screen.blit(txt_start, txt_start.get_rect(center=(WIDTH//2, HEIGHT//2 + 160)))
    ...
    while True:
        ...
        while gameplay_running:
            ...
            if is_paused:
                ...
                txt_quit = font_math.render("> ABORT MISSION <" if pause_selection == 1 else "  ABORT MISSION  ", True, quit_color)

                screen.blit(txt_res, txt_res.get_rect(center=(WIDTH//2, HEIGHT//2 - 10)))
                screen.blit(txt_quit, txt_quit.get_rect(center=(WIDTH//2, HEIGHT//2 + 40)))

                txt_nav_hint = font_ui.render("[W/S or UP/DOWN] Nav  |  [ENTER] Select", True, GRAY)
            ...
            for a in aliens[:]:
                ...
                if a.y > HEIGHT - 70 and not death_sequence:
                    ...
                    shockwaves.append(Shockwave(turret_pos[0], turret_pos[1], WHITE, speed=8))
                    for _ in range(120): 
                        particles.append(Particle(turret_pos[0], turret_pos[1], random.choice([CYAN, WHITE, ORANGE, RED]), speed_range=(5, 20), size_range=(3, 8)))
            ...
            if not death_sequence:
                draw_massive_carrier(screen, turret_pos, active_alien)
            ...
            if screen_offset != [0, 0]:
                ...
                screen.fill(BLACK)
                screen.blit(screen_copy, screen_offset)

            pygame.display.flip()

        # --- STATE 3: GAME OVER SCREEN ---
        sim_dict = {}
        for item in load_leaderboard():
            n = item["name"]
            if n != username: 
                if n not in sim_dict or item["score"] > sim_dict[n]["score"]:
                    sim_dict[n] = item
                    
        sim_dict[username] = {"name": username, "score": score, "level": level}
        temp_data = sorted(sim_dict.values(), key=lambda x: x["score"], reverse=True)
        
        player_rank = -1
        for idx, item in enumerate(temp_data):
            if item["name"] == username and item["score"] == score:
                player_rank = idx + 1
                break

        top_scores = save_leaderboard(username, score, level)
        ...
        while over_loop:
            ...
            pygame.draw.line(screen, CYAN, (0, HEIGHT - 120), (WIDTH, HEIGHT - 120), 2)

            txt_over = font_big.render("BASE DESTROYED", True, RED)
            txt_p_score = font_math.render(f"YOUR SCORE : {score}", True, YELLOW)
            txt_p_lvl = font_math.render(f"FINAL LEVEL: {level}", True, WHITE)
            
            if player_rank <= 5:
                txt_p_rank = font_math.render(f"RANKING    : #{player_rank}", True, GREEN)
            else:
                txt_p_rank = font_math.render("RANKING    : TIDAK MASUK 5 BESAR", True, GRAY)
            
            screen.blit(txt_over, (50, HEIGHT // 2 - 120))
            screen.blit(txt_p_score, (50, HEIGHT // 2 - 20))
            screen.blit(txt_p_lvl, (50, HEIGHT // 2 + 20))
            screen.blit(txt_p_rank, (50, HEIGHT // 2 + 60))

            txt_board_title = font_math.render("GLOBAL LEADERBOARD (TOP 5)", True, CYAN)
            screen.blit(txt_board_title, (WIDTH // 2 + 40, HEIGHT // 2 - 140))
            
            start_y = HEIGHT // 2 - 90
            for idx, data in enumerate(top_scores[:5]):
                is_current = (data["name"] == username)
                color_rank = GREEN if is_current else WHITE
                
                rank_str = f"{idx+1}. {data['name'].ljust(10)} - {str(data['score']).zfill(5)} (LVL {data['level']})"
                screen.blit(font_math.render(rank_str, True, color_rank), (WIDTH // 2 + 40, start_y + (idx * 40)))

            txt_hint = font_ui.render("Press 'R' to RESTART   |   Press 'ESC' to QUIT", True, WHITE)
            screen.blit(txt_hint, txt_hint.get_rect(center=(WIDTH//2, HEIGHT - 60)))
